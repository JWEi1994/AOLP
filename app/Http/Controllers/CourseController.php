...
class CourseController extends Controller
{
    ...
    public function instructorCourseVideoSave(Request $request)
    {
        ...
        if ($courseVideos->save()) {
            ...
            return $this->return_output('flash', 'success', 'Upload successfully', 'instructor-course-video/'.$course_id, '200');
            // return $this->return_output('flash', 'success');
        } else {
            $return_data = array(
                'status' => false,
            );
    ...
    public function postLectureResourceSave($lid, Request $request)
    {
        ...
        if ($courseFiles->save()) {
            ...
            $return_data = array(
                ...
                'file_title' => $file_title,
                'file_size' => \aolpHelpers::HumanFileSize($file_size)
            );
        } else {
            $return_data = array(
                'status' => false,
