    ...
    <div class="page-content">
        ...
        <div class="panel">
            <div class="panel-body">
                ...
                <form method="POST" action="{{ route('instructor.profile.save') }}" id="profileForm"
                    ...
                    <div class="row">
                        ...
                        <div class="form-group col-md-4">
                            ...
                            @if ($errors->has('contact_email'))
                                <label class="error"
                                    for="contact_email">{{ $errors->first('contact_email') }}</label>
                            @endif
                        </div>
                        ...
                        <div class="form-group col-md-4">
                            ...
                            <input type="text" class="form-control" name="paypal_id" placeholder="Paypal ID"
                                value="{{ $instructor->paypal_id }}" />
                        ...
                        <div class="form-group col-md-6">
                            ...
                            <input type="text" class="form-control" name="link_facebook" placeholder="Facebook Link"
                                value="{{ $instructor->link_facebook }}" />
                            @if ($errors->has('link_facebook'))
                                <label class="error"
                                    for="link_facebook">{{ $errors->first('link_facebook') }}</label>
                            @endif
                        </div>

                        <div class="form-group col-md-6">
                            <label class="form-control-label">Linkedin Link </label>
                            <input type="text" class="form-control" name="link_linkedin" placeholder="Linkedin Link"
                                value="{{ $instructor->link_linkedin }}" />
                            @if ($errors->has('link_linkedin'))
                                <label class="error"
                                    for="link_linkedin">{{ $errors->first('link_linkedin') }}</label>
                            @endif
                        </div>

                        <div class="form-group col-md-6">
                            <label class="form-control-label">Twitter Link </label>
                            <input type="text" class="form-control" name="link_twitter" placeholder="Twitter Link"
                                value="{{ $instructor->link_twitter }}" />
                            @if ($errors->has('link_twitter'))
                                <label class="error"
                                    for="link_twitter">{{ $errors->first('link_twitter') }}</label>
                            @endif
                        </div>

                        <div class="form-group col-md-6">
                            <label class="form-control-label">Google Plus Link </label>
                            <input type="text" class="form-control" name="link_googleplus" placeholder="Google Plus Link"
                                value="{{ $instructor->link_googleplus }}" />
                            @if ($errors->has('link_googleplus'))
                                <label class="error"
                                    for="link_googleplus">{{ $errors->first('link_googleplus') }}</label>
                            @endif
                        </div>
                    ...
                    <div class="row">
                            ...
                            <label class="cabinet center-block">
                                <figure class="course-image-container">
                                    <i data-toggle="tooltip" data-original-title="Delete" data-id="course_image"
                                        ...
                                        data-content="{{ Crypt::encryptString(json_encode(['model' => 'courses', 'field' => 'course_image', 'pid' => 'id', 'id' => $instructor->id, 'photo' => $instructor->instructor_image])) }}"
                                        style="display: @if (Storage::exists($instructor->instructor_image)){{ 'block' }} @else {{ 'none' }} @endif"></i>
                                    <img src="@if (Storage::exists($instructor->instructor_image)){{ asset('storage/' . $instructor->instructor_image) }}@else{{ asset('backend/assets/images/no_image.jpg') }}@endif" class="gambar img-responsive"
                        ...
                        <div class="form-group col-md-8">
                            <label class="form-control-label">Bio <span class="required">*</span></label>
...
@section('javascript')
    <script type="text/javascript">
        $(document).ready(function() {
            //image crop start
            $(".gambar").attr("src", @if (Storage::exists($instructor->instructor_image))"{{ asset('storage/' . $instructor->instructor_image) }}" @else "{{ asset('backend/assets/images/no_image.jpg') }}" @endif);
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            $("#" + current_id + "-output").attr("src",
                                "{{ asset('backend/assets/images/no_image.jpg') }}"
                            );
                            this_id.hide();
                        }
                    });
